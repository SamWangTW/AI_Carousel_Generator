...
def build_caption_from_plan_prompt(slide_plans: list[dict], tone: str) -> list[dict]:
    tone_instruction = TONE_INSTRUCTIONS.get(tone, TONE_INSTRUCTIONS["educational"])

    plan_summary = "\n".join(
        f"  Slide {p['index']} ({p['role']}): {p['idea']}" for p in slide_plans
    )
    ...
    user_message = f"""Write an Instagram caption for a carousel post based on the following slide plan:
